...
| H-7 | ~~Audit log written AFTER delete (`lib/actions/vendors.ts:89-95`, `lib/actions/users.ts:173-175`)~~ | If logging fails, who deleted the record is lost | **FIXED** — moved `logAudit` to before the delete in both actions |
| H-8 | ~~IDOR in `reviewAction` — `responseId` not cross-checked against `assessmentId` (`lib/actions/collaboration.ts:54-107`)~~ | Attacker passes `assessmentId=A, responseId=B` where B belongs to assessment C; review decision applied to wrong assessment's response | **FIXED** — now looks up response and verifies `response.assessmentId === assessmentId` before proceeding |
| H-9 | ~~IDOR in certification/attachment actions — no ownership verification (`lib/actions/certifications.ts`, `lib/actions/vendors.ts:241-261`)~~ | Any user with `VENDORS_EDIT` could update/delete any vendor's certifications or attachments regardless of ownership | **FIXED** — `saveCertificationAction` verifies cert belongs to vendor; `removeAttachmentAction` resolves entity type and verifies vendor ownership; `removeVendorAttachmentAction` verifies `entityType === "Vendor"` and `entityId === vendorId` |
| H-10 | ~~Missing input validation — `addUserAction` uses manual truthiness checks only (`lib/actions/users.ts:42`)~~ | Invalid emails, 1-character passwords accepted and persisted | **FIXED** — `addUserAction` now uses `userCreateSchema` (zod: email + min 12 char password + non-empty name/roleId); resolves M-5 as well |
| H-11 | ~~RISK_ACCEPTED findings silently deleted on rescore (`lib/db/scoring.ts:78-88`) — reconciliation deletes ALL findings on a response when answer becomes compliant~~ | Reviewer's explicit `RISK_ACCEPTED` finding vanishes on vendor resubmit; finding history silently lost; partial rescore from a single-answer update (not full submit) also triggers this | **FIXED** — `deleteMany` now filters `status: "OPEN"` only; `RISK_ACCEPTED` and `REMEDIATED` findings survive rescore |
| H-12 | ~~Rate limiter per-prefix bypass (`lib/api-auth.ts:65`)~~ | Attacker cycles generated prefixes for unlimited attempts | **FIXED** — added IP-based rate limit (`apikey-ip`) as fallback before the prefix-based check |
| H-13 | ~~Rate limiter FIFO eviction (`lib/rate-limit.ts`)~~ | Attacker floods 50K unique entries to evict all legitimate rate-limit keys | **FIXED** — replaced FIFO eviction with per-namespace 5K cap + global 100K hard limit (reject when full) |
| H-14 | ~~DB record deleted before storage files (`lib/db/assessments.ts:195-196`, `lib/db/vendors.ts:145-158`)~~ | Crash between DB delete and file cleanup → permanently orphaned files with no DB reference to reconnect them | **FIXED** — `deleteAssessment` and `deleteVendor` now delete storage files first, then the DB record |
...
| M-8 | ~~Content-Disposition header injection~~ | CRLF injection allows crafting arbitrary HTTP response headers | **FIXED** — vendor name and attachment fileName wrapped in `encodeURIComponent()` |
| M-9 | ~~Page parameter `NaN` propagation~~ | `?page=abc` causes Prisma to receive NaN | **FIXED** — 3 API routes now use `parseInt(..., 10) \|\| 1` |
| M-10 | ~~Unbounded `pageSize` in audit endpoint (`app/api/v1/audit/route.ts:25-27`)~~ | `?pageSize=10000000` fetches millions of rows | **FIXED** — added `Math.min(parsedPageSize, 500)` |
| M-11 | ~~`AUTH_SECRET` minimum length is 1 character (`lib/env.ts:9`)~~ | Auth.js uses this to sign JWT session tokens | **FIXED** — changed to `.min(32)` matching `APP_ENCRYPTION_KEY` |
| M-12 | ~~Delete-user UX: no pre-deletion summary of orphaned relations (`lib/db/users.ts:91-93`)~~ | User deleted with no awareness of what will be orphaned; no opportunity to reassign | **FIXED** — two-phase delete flow: first click fetches `getUserDeletionImpact` (vendors, assessments, findings, reviews, API keys count), second click confirms deletion with a detailed breakdown of exactly what will be orphaned; `getUserDeletionImpactAction` returns `DeletionImpact` type; `ConfirmDialog` now accepts `ReactNode` descriptions |
| M-13 | ~~Portal submission: no maximum answer value length validation~~ | Attacker can submit a 1MB string as a single answer | **FIXED** — `z.string().max(50000)` on portal answer value schema |
| M-14 | ~~Notification counts: `userId` parameter ignored for 3 of 4 queries~~ | API says "for user X" but returns org-wide data | **FIXED** — removed misleading `userId` parameter; all counts are now explicitly org-wide |
| M-15 | ~~`computeTotalScore` returns `0` for all-N/A assessments (`lib/scoring.ts:146-149`)~~ | Callers couldn't distinguish "nothing to score" from "0% compliance" | **FIXED** — returns `null` when `totalMax === 0`; Prisma Float accepts null; dashboard already filters null scores |
| M-16 | ~~Portal password uses bcrypt at 10 rounds (vs 12 elsewhere) — `sendAssessment` passes `10` (`lib/db/assessments.ts:242`)~~ | Slightly weaker brute-force resistance for portal password gate | **FIXED** — bumped to 12 rounds for consistency with user passwords. bcrypt `compare` reads rounds from the hash prefix, so existing 10-round hashes continue to verify correctly (resolves both M-16 and L-1) |
...
| M-20 | ~~`submitAssessment` uses plaintext `accessToken` lookup (`lib/db/assessments.ts`)~~ | Incomplete M-17 fix — portal submission path still queried by plaintext token instead of hashing first. If the DB is compromised, plaintext tokens in the `accessToken` column grant direct portal access without needing to crack SHA-256 hashes. `getAssessmentByToken` and `getAssessmentForToken` were fixed in M-17 to use `tokenHash` lookups, but `submitAssessment` was missed | **FIXED** — changed `where: { accessToken: token }` to `where: { tokenHash: hashToken(token) }` in `submitAssessment`, matching the pattern used by all other token lookup functions |
...
| L-16 | ~~No secret rotation mechanism for `APP_ENCRYPTION_KEY`~~ | Changing the encryption key permanently destroys all encrypted secrets (SMTP password, SSO client secrets, cloud storage credentials). Recovery requires restoring the old key or manually re-entering every encrypted secret. There is no warning UI before a key change, no key versioning, and no re-encryption tool | **FIXED** — warnings added to `docker-compose.yml` and deployment docs (`docs/deployment/docker.md`) with safe rotation and recovery procedures. Appropriate for Low severity — the key is an environment variable, not a DB setting that can be toggled in-app |
...
**Last reviewed:** July 2026 (L-17 fix — cron timestamp tracking, plus considerations audit)
**App version:** 0.1.0
