...
| H-6 | `importFrameworkAction` — no transaction between `createFramework` and `createControls` (`lib/actions/frameworks.ts:113-128`) | Controls creation fails → framework persists with zero controls | Wrap both in `prisma.$transaction` |
